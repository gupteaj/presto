        ...
        extends SqlBaseBaseVisitor<Node>
{
    ...
    public Node visitTableVersion(SqlBaseParser.TableVersionContext context)
    {
        ...
        switch (context.tableVersionType.getType()) {
            ...
            case SqlBaseLexer.TIMESTAMP:
                return timestampExpression(getLocation(context), getTableVersionState((Token) context.tableVersionState().getChild(0).getPayload()), child);
            case SqlBaseLexer.SYSTEM_VERSION:
            case SqlBaseLexer.VERSION:
                return versionExpression(getLocation(context), getTableVersionState((Token) context.tableVersionState().getChild(0).getPayload()), child);
            default:
    ...
    private static TableVersionState getTableVersionState(Token token)
    {
        switch (token.getType()) {
        ...
        throw new IllegalArgumentException("Unsupported table version state: " + token.getText());
    }
